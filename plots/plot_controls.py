...
#w_norm = [dump[k].abs().sum()+dump[2*time_steps-k].abs().sum() for k in range(0, 2*time_steps, 2)]
#b_norm = [dump[k].abs().sum()+dump[2*time_steps-k].abs().sum() for k in range(0, 2*time_steps) if k%2==1]
