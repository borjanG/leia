...
class Dynamics(nn.Module):
    """ $f(theta(t), x(t)) in the neural ODE.
    ...
    def __init__(self, 
        ...
        if non_linearity not in activations.keys() \
            or architecture not in architectures.keys():
            raise ValueError("Activation function or architecture not found. \
                             Please reconsider.")
        ...
        if self.architecture > 0:
            ...
            blocks1 = [nn.Linear(self.input_dim, hidden_dim) 
                       for _ in range(self.time_steps)]
            self.fc1_time = nn.Sequential(*blocks1)
            ##-- R^{d_hid} -> R^{d_aug} layer --
            blocks3 = [nn.Linear(hidden_dim, self.input_dim) 
                       for _ in range(self.time_steps)]
...
class Semiflow(nn.Module):
    """Given f, generate semiflow by solving x'(t) = f(theta(t), x(t)).
    ...
    def forward(self, x, eval_times=None):
        ...
        if self.adjoint:
            ...
            out = odeint(self.dynamics, x_aug, 
                         integration_time, 
                         method='midpoint', 
                         options={'step_size': dt})
        if eval_times is None:
...
class NeuralODE(nn.Module):
    """Returns the flowmap of the neural ODE, i.e. the solution to the neural ODE, or the
    ...
    def __init__(self, 
        ...
        if output_dim == 1 and cross_entropy: 
            # output_dim = 1 pour MSE; >=2 
            # pour cross entropy for binary classification.
            raise ValueError('Incompatible output dimension with loss.')
